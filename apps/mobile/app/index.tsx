...
// Advanced Home Screen (single, stable component) with typing-safe inputs + pending tx Boost/Cancel
...
export default function Index() {
  ...
  // Typing-safe: keep input text in refs (no state churn while typing)
  const toRef = useRef("");
  ...
  // Pause polling while modals open
  const pausePollingRef = useRef(false);

  const T = themeFor(theme);

  // Background
  const matrixBg = useMemo(
    () => require("../src/app/assets/skins/matrix-honeycomb.png"),
    []
  );
  ...
  useEffect(() => {
    // Don’t animate while editing; it can cause focus churn on some WebKit builds.
    if (isEditing) {
      pulse.stopAnimation();
      pulse.setValue(0);
      return;
  ...
  function Card(props: { children: React.ReactNode; style?: any }) {
    return (
      <GlassCard
        style={[{ borderWidth: 1, borderColor: T.border, backgroundColor: T.glass }, props.style]}
      >
  ...
  useEffect(() => {
    (async () => {
      ...
      if (savedSkin === "matrix-honeycomb" || savedSkin === "solid-noir") {
  ...
  function computeTipAmount(): number {
    ...
    if (tipMode === "small") return ONE_SAT;
    if (tipMode === "medium") return Number((ONE_SAT * 10).toFixed(8));
    if (tipMode === "large") return Number((ONE_SAT * 100).toFixed(8));
  ...
  const pendingTxs = useMemo(
    () => (txs || []).filter((t: any) => String(t?.status || "") === "pending"),
    [txs]
  );
  ...
  const content = (
    ...
    >
      {/* Banner for sanity */}
      <Text style={{ color: T.sub, marginTop: 10, fontWeight: "900" }}>INDEX_CLEAN_ADVANCED ✅</Text>
      ...
      <Card style={{ marginTop: 12 }}>
        ...
        <Text style={{ color: T.text, fontSize: 16, fontWeight: "900", marginTop: 4 }}>
        ...
        {txs.length === 0 ? (
          ...
          <View style={{ gap: 10 }}>
            {txs.slice(0, 12).map((tx: any, idx: number) => {
              ...
              const isPendingTx = status === "pending";
              return (
                <View
                ...
                >
                  ...
                  <Text style={{ color: T.sub, marginTop: 4, fontWeight: "800" }}>
                    Nonce: {Number(tx?.nonce ?? "—")} • Gas: {Number(tx?.gasFee || 0)} • Service: {Number(tx?.serviceFee || 0)}
                  </Text>
        ...
        >
          <GlassCard style={{ borderWidth: 1, borderColor: T.border, backgroundColor: T.glass }}>
            <View style={{ padding: 14 }}>
              ...
              <Text style={{ color: T.sub, marginTop: 10, fontWeight: "800" }}>
                Cancel replaces the pending tx with a self-send using higher gas.
              </Text>
